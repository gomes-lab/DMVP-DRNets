...
def main(_):
    ...
    for epoch in range(max_epoch):
        ...
        if (epoch + 1) % FLAGS.save_epoch == 0: #exam the model on validation set
            ...
            current_loss, preds, Ys = validation_step(sess, hg, DL, merged_summary, summary_writer, test_idx, epoch, metrics, metric_names)

            if (True or current_loss < best_loss): # no model selection, run to a fixed epoch 
