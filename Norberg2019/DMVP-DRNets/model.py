...
class MODEL:
	...
	def __init__(self, is_training, n_feature, n_classes):

		tf.set_random_seed(19950420)

		r_dim = n_classes
		
		self.input_X = tf.placeholder(dtype=tf.float64,shape=[None, n_feature],name='input_X')
		
		self.input_Y = tf.placeholder(dtype=tf.float64,shape=[None, n_classes],name='input_Y')

		self.keep_prob = tf.placeholder(tf.float64) #keep probability for the dropout

		weights_regularizer = slim.l2_regularizer(FLAGS.weight_decay)

		############## compute mu & sigma ###############

		self.fc_1 = slim.fully_connected(self.input_X, 128, weights_regularizer=weights_regularizer, scope='extractor/fc_1')
		self.fc_2 = slim.fully_connected(self.fc_1, 256, weights_regularizer=weights_regularizer, scope='extractor/fc_2')
		self.fc_3 = slim.fully_connected(self.fc_2, 256, weights_regularizer=weights_regularizer, scope='extractor/fc_3')

		feature=self.fc_3			
		
		#initialize the square root of the residual covariance matrix 
		self.r_sqrt_sigma=tf.Variable(np.random.uniform(-np.sqrt(6.0/(r_dim+FLAGS.z_dim)), np.sqrt(6.0/(r_dim+FLAGS.z_dim)), (r_dim, FLAGS.z_dim)), dtype=tf.float64, name='r_sqrt_sigma')
		...
		self.covariance=self.sigma + tf.eye(r_dim, dtype = tf.float64)



		self.cov_diag = tf.diag_part(self.covariance)
		# r_mu
		self.normalized_mu = slim.fully_connected(feature, r_dim, weights_regularizer=weights_regularizer, activation_fn = None, scope='extractor/normalized_mu')
		#self.normalized_mu = tf.clip_by_value(self.normalized_mu, -3, 3) # within [-3*std_err, 3*std_err]
		self.r_mu = self.normalized_mu * tf.sqrt(self.cov_diag)

		#compute the mean of the normal random variables 
		#self.r_mu = slim.fully_connected(feature, r_dim, activation_fn=None, weights_regularizer=weights_regularizer, scope='r_mu')
		...
		self.eps1=tf.constant(1e-6, dtype="float64")
		self.eps3=1e-30
		############## alpha tuning##############
		...
		self.noise = tf.random_normal(shape=[n_sample, tf.shape(self.r_mu)[0], FLAGS.z_dim], dtype = tf.float64)
		...
		E = tf.cast(norm.cdf(tf.cast(self.sample_r, tf.float32)), tf.float64) * (1-self.eps1)+self.eps1*0.5

		self.E=E

		#compute the loss for each sample point
		self.sample_nll = tf.negative((tf.log(E)*self.input_Y+tf.log(1-E)*(1-self.input_Y)), name='sample_nll')
		...
		#self.indiv_prob = tf.reduce_mean(E , axis=0, name='individual_prob')

		self.indiv_prob = tf.cast(norm.cdf(tf.cast(self.normalized_mu, tf.float32)), tf.float64) * (1-self.eps1) + 0.5 * self.eps1

		cross_entropy = (tf.log(self.indiv_prob)*self.input_Y + tf.log(1 - self.indiv_prob)*(1 - self.input_Y))
		self.marginal_loss = - tf.reduce_mean(tf.reduce_sum(cross_entropy, axis = 1))
		
		###### loss ##############
		tf.summary.scalar('train/nll_loss',self.nll_loss)
		tf.summary.scalar('train/marginal_nll_loss',self.marginal_loss)
