...
def eval(exp, num, path):
    ...
    pred = pred[-test_Y.shape[0]:, :, :] #use the valiation performance
    pred = np.transpose(pred, [1, 0, 2]) #n_sample, n_data, n_sp
    prob = pred * test_Y + (1 - pred) * (1 - test_Y)
    prob = prob * (1 - 1e-2) + 0.5 * 1e-2
    nll = np.mean(np.sum(-np.log(prob), axis = 2))
    print("nll_loss", nll)
...
exps = ["BBS"] #["birds", "butterfly", "plant", "trees", "vegetation"]
for dataset in exps:
    for i in range(1, 2):
        eval(dataset, i, "NNGP/")
